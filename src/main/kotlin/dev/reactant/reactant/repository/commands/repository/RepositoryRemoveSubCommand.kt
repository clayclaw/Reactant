...
@CommandLine.Command(
    ...
    description = ["Remove existing repositories by name"]
)
class RepositoryRemoveSubCommand(private val repositoryService: RepositoryService) :
    ReactantCommand(ReactantPermissions.REPOSITORY.MODIFY.toString()) {
    @CommandLine.Parameters(
        arity = "1..*",
        paramLabel = "NAME",
        description = ["Name of the repositories, wildcard is available"]
    )
